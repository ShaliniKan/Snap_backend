...
const OrderDetail = () => {
    ...
    if (loading) {
        return (
            <AccountPageLayout pageTitle="My Orders" breadcrumbCurrent="Order Details">
                <SectionState>Loading order details...</SectionState>
            </AccountPageLayout>
        );
    }
    ...
    if (!order) {
        return (
            <AccountPageLayout pageTitle="My Orders" breadcrumbCurrent="Order Details">
                <SectionState>Order not found.</SectionState>
            </AccountPageLayout>
        );
    }
    ...
    return (
        ...
        <div className="space-y-6">
            <div>
                <Link className="text-[12px] text-[#2f82c6] hover:underline" to={ROUTES.customer.orders}>
                    ← Back to orders
                </Link>
                <p className="mt-3 text-[14px] font-normal text-[#333333]">
                    Order #{order._id.slice(-8).toUpperCase()}
                </p>
                <p className="mt-1 text-[12px] text-[#666666]">{new Date(order.createdAt).toLocaleString("en-IN")}</p>
            </div>
